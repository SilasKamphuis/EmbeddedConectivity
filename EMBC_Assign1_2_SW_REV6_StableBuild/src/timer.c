...
//register function
//T2callbackflag will return 1 when callback function is called
void timer_register_T2callback(T2callback ptr_T2callback) { 
    T2callbackfp = ptr_T2callback; 
    T2callbackflag = 1; 
} 

//Interrupt service routine of the timer channel
//The function pointer is called, because of this the function in the higher-level block will execute when the interrupt occurs.
