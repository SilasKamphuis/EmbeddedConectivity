 ...
 ****************************************************************************/
    ...
    #endif
        static HTTP_IO_RESULT HTTPPostVendMachine(HTTP_CONN_HANDLE connHandle);
#endif
...
HTTP_IO_RESULT TCPIP_HTTP_GetExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    if(!memcmp(filename, "webvend/lights.htm", 18))
    {
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"lights");
        if(ptr && !memcmp(ptr, "on", 2)) 
            VendSetLights(true);
        else if(ptr && !memcmp(ptr, "off", 3))
            VendSetLights(false);

    }

    // If its the forms.htm page.
    else if(!memcmp(filename, "demo/forms.htm", 14))
    {
...
//            BSP_LEDStateSet(APP_TCPIP_LED_2, (*ptr == '1'));
    ...
    else if(!memcmp(filename, "demo/cookies.htm", 16))
    {
    ...
    else if(!memcmp(filename, "demo/leds.cgi", 13))
    {
...
#if defined(SYS_OUT_ENABLE)
    if(!memcmp(filename, "demo/forms.htm", 9))
        return HTTPPostLCD(connHandle);
#endif

#if defined(HTTP_APP_USE_MD5)
    if(!memcmp(filename, "demo/upload.htm", 10))
        return HTTPPostMD5(connHandle);
#endif

#if defined(HTTP_APP_USE_RECONFIG)
    if(!memcmp(filename, "demo/protect/config.htm", 18))
        return HTTPPostConfig(connHandle);
    #if defined(TCPIP_STACK_USE_SNMP_SERVER)
    else if(!memcmp(filename, "demo/snmp/snmpconfig.htm", 19))
        return HTTPPostSNMPCommunity(connHandle);
    #endif
#endif

#if defined(TCPIP_STACK_USE_SMTP_CLIENT)
    if(!strcmp((char *)filename, "demo/email/index.htm"))
        return HTTPPostEmail(connHandle);
#endif

#if defined(TCPIP_STACK_USE_DYNAMICDNS_CLIENT)
    if(!strcmp((char *)filename, "demo/dyndns/index.htm"))
        return HTTPPostDDNSConfig(connHandle);
#endif

    if(!strcmp((char *)filename, "webvend/products.htm"))
        return HTTPPostVendMachine(connHandle);
...
static HTTP_IO_RESULT HTTPPostVendMachine(HTTP_CONN_HANDLE connHandle)
{
    uint8_t *httpDataBuff;
    uint8_t smPost;
    static uint8_t item = 0u;
    static uint8_t index = 0;
    float dollarPrice = 0;
    uint8_t quarterPrice = 0;

    #define ITEM_NONE  (0u)
    #define ITEM_NAME  (1u)
    #define ITEM_PRICE (2u)

    #define SM_VENDMACHINE_READ_NAME       (0u)
    #define SM_VENDMACHINE_READ_VALUE      (1u)
    #define SM_VENDMACHINE_DONE            (2u)
    ...
    switch(smPost)
    {
        ...
        case SM_VENDMACHINE_READ_NAME:
            // If all parameters have been read, end
            if(TCPIP_HTTP_CurrentConnectionByteCountGet(connHandle) == 0u)
            {
                TCPIP_HTTP_CurrentConnectionPostSmSet(connHandle, SM_VENDMACHINE_DONE);
                break;
            }
            ...
            if(memcmp((void *)httpDataBuff, (const void *)"name", 4) == 0)
            {
                item = ITEM_NAME;
                index = httpDataBuff[5] - '0';
            }
            else if(memcmp((void *)httpDataBuff, (const void *)"price", 5) == 0)
            {
                item = ITEM_PRICE;
                index = httpDataBuff[6] - '0';
            } else
                item = ITEM_NONE;

            // Move to reading the value for Products[i].name / Products[i].price
            TCPIP_HTTP_CurrentConnectionPostSmSet(connHandle, ++smPost);

        // Reads in values and assigns them to the DDNS RAM
        case SM_VENDMACHINE_READ_VALUE:
            // Read a value
            if(TCPIP_HTTP_PostValueRead(connHandle, httpDataBuff, TCPIP_HTTP_MAX_DATA_LEN) == HTTP_READ_INCOMPLETE)
                return HTTP_IO_NEED_DATA;

            if(item == ITEM_NAME)
                strcpy(Products[index].name, (char *)httpDataBuff);
            else if(item == ITEM_PRICE)
            {
                    dollarPrice = atof((char *)httpDataBuff);
                    quarterPrice = (uint8_t) ceil(dollarPrice/0.25);
                    if (quarterPrice > 20)
                        quarterPrice = 20;
                    Products[index].price = quarterPrice;
            }

            // Return to reading names
            TCPIP_HTTP_CurrentConnectionPostSmSet(connHandle, SM_VENDMACHINE_READ_NAME);
            break;

        // Sets up the DDNS client for an update
        case SM_VENDMACHINE_DONE:
            // Redirect to prevent POST errors
            lastSuccess = true;
            strcpy((char *)httpDataBuff, "/webvend/products.htm");
...
static HTTP_IO_RESULT HTTPPostLCD(HTTP_CONN_HANDLE connHandle)
{
    ...
    switch(TCPIP_HTTP_CurrentConnectionPostSmGet(connHandle))
    {
        ...
        case SM_POST_LCD_READ_VALUE:
            ...
            strcpy((char *)httpDataBuff, "/demo/forms.htm");
            TCPIP_HTTP_CurrentConnectionStatusSet(connHandle, HTTP_REDIRECT);
            return HTTP_IO_DONE;
    }
...
static HTTP_IO_RESULT HTTPPostConfig(HTTP_CONN_HANDLE connHandle)
{
    ...
    if(bConfigFailure == false)
    {
        ...
        strcpy((char *)httpDataBuff, "/demo/protect/reboot.htm?");
    ...
    {   // Configuration error
        ...
        if(httpDataBuff)
        {
            strcpy((char *)httpDataBuff, "/demo/protect/config.htm");
        }
    }
...
//            TCPIP_SMTP_Put(BSP_LEDStateGet(APP_TCPIP_LED_3) + '0');
        ...
        case SM_EMAIL_FINISHING:
            ...
            if(!TCPIP_SMTP_IsBusy())
            {
                ...
                strcpy((char *)httpDataBuff, "/demo/email/index.htm");
                TCPIP_HTTP_CurrentConnectionStatusSet(connHandle, HTTP_REDIRECT);
                return HTTP_IO_DONE;
            }
...
static HTTP_IO_RESULT HTTPPostDDNSConfig(HTTP_CONN_HANDLE connHandle)
{
    ...
    switch(smPost)
    {
        ...
        case SM_DDNS_DONE:
            ...
            // Redirect to prevent POST errors
            lastSuccess = true;
            strcpy((char *)httpDataBuff, "/demo/dyndns/index.htm");
            TCPIP_HTTP_CurrentConnectionStatusSet(connHandle, HTTP_REDIRECT);
            return HTTP_IO_DONE;
    }
...
void TCPIP_HTTP_Print_pot(HTTP_CONN_HANDLE connHandle)
{
    ...
    if(SWT_GetValue(0))
    {
        uitoa(1, (uint8_t *)AN0String);
    } else {
        uitoa(0, (uint8_t *)AN0String);
    }
...
void TCPIP_HTTP_Print_machineDesc(HTTP_CONN_HANDLE connHandle)
{
    ...
    if(strlen((char *)machineDesc) > TCPIP_TCP_PutIsReady(sktHTTP))
    {
        callbackPos = 0x01;
    } else {
        callbackPos = 0x00;
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)machineDesc);
    }
    TCPIP_HTTP_CurrentConnectionCallbackPosSet(connHandle, callbackPos);
}

void TCPIP_HTTP_Print_name(HTTP_CONN_HANDLE connHandle,uint16_t item)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);

    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) Products[item].name);
}

void TCPIP_HTTP_Print_stock(HTTP_CONN_HANDLE connHandle,uint16_t item)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    uint8_t digits[4];
...
void TCPIP_HTTP_Print_price(HTTP_CONN_HANDLE connHandle,uint16_t item)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    char digits[4];
    double dollarPrice = 0;

    dollarPrice = Products[item].price * 0.25;
    sprintf(digits, "%.2f", dollarPrice);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) digits);
}

void TCPIP_HTTP_Print_status(HTTP_CONN_HANDLE connHandle,uint16_t item)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);

    if (Products[item].stock < 10)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "low");
    else
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "ok");
}

void TCPIP_HTTP_Print_lights_chk(HTTP_CONN_HANDLE connHandle,uint16_t on)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);

    if (on && !VendGetLights())
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "checked");
    else if (!on && VendGetLights())
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "checked");
}
...
void TCPIP_HTTP_Print_angle(HTTP_CONN_HANDLE connHandle)
{
    ...
    (void)itoa(digits, servo_getAngle(), 10);
    TCPIP_TCP_StringPut(sktHTTP, (uint8_t *)digits);
}
...
void TCPIP_HTTP_Print_distance(HTTP_CONN_HANDLE connHandle)
{
    ...
    uitoa(srf_getDistanceValue(), digits);
    TCPIP_TCP_StringPut(sktHTTP, digits);
}
